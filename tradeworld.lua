...
local r = {
    finder = { find_petlist = {}, find_weight = 1, find_mutation = {}, find_enabled = false, find_tp = false, find_tp_every_mins = 10 },
    showcase = { fruit_list = {}, pet_list = {} },
    sameserver_buyonly = false,
    show_player_stats = true,
    scantargetpets = {},
    manual_min_weight = .1,
    manual_showbaseweight = true,
    fixkgbug_easter = false,
    find_settings = {},
    ...
    listing_min_level = 1,
    listing_max_level = 1,
    listing_min_weight = .9,
    listing_max_weight = 2.86,
    listing_token_price = 99,
    listing_auto_unfav = false,
    skin_booth_list = {},
    auto_claim_booth = false,
    auto_equip_big_pet = false,
    ...
    autoreconnect_tries = 0,
    was_auto_reconnect = false,
    auto_promote_listing = false,
    removelistingpetsfilter = {},
    sellfruit = { fruit_list_allow = {}, is_fruit_enabled = false, fruit_price = 30, fruit_min_weight = .03, fruit_max_weight = 67.67, fruit_mutations = {}, fruit_auto_fav = false }
}
local Y = "exotichub99"
...
Z.LoadMainGameAgeBreakSettingsReadOnly = function()
    ...
    Z.AgeBreakMainSettingsReadOnly = {
        is_active_agebreak = c.is_active_agebreak == true,
        target_team = type(c
            .target_team) == "table" and c.target_team or {},
        dup_team = type(c.dup_team) == "table" and c.dup_team or {},
        claim_team =
            type(c.claim_team) == "table" and c.claim_team or {},
        submit_team = type(c.submit_team) == "table" and c.submit_team or
            {},
        idle_team = type(c.idle_team) == "table" and c.idle_team or {},
        max_level = tonumber(c.max_level) or 125,
        use_filters =
            c.use_filters == true,
        avoid_age_filter = c.avoid_age_filter == true,
        avoid_weight_filter = c.avoid_weight_filter ==
            true,
        auto_skip_tokens = c.auto_skip_tokens == true,
        autorejoinagebreak = c.autorejoinagebreak == true
    }
...
J.task_data_sync1 = task.spawn(function()
    while true do
        task.wait(1.5)
...
J.CopyToClipBoard = function(G)
    if setclipboard then
        ...
        Z.Notify("\226\157\140 Clipboard copy not supported", 3)
    end
end
...
E.time = {}
E.time = {
    GetMinsFromSecs = function(G) return h(G) / 60 end,
    GetSecsFromMin = function(G) return h(G) * 60 end,
    GetSecsFromHours = function(
        G)
        return h(G) * 3600
    end,
    GetHoursFromSecs = function(G) return h(G) / 3600 end,
    FormatMS = function(G)
        local V = h(G)
    ...
    FormatText = function(G)
        ...
        if V < 60 then
            return tostring(V) .. "s"
        elseif V < 3600 then
            return string.format("%dm %ds", math.floor(V / 60),
                V % 60)
        else
...
J.AllPetPassiveData = {}
J.PetDataAll = {}
J.GetAllPetDataPassives = function()
    for G, V in pairs(V.PetList) do
...
u.UpdatePlayerStatusUI = function()
    if r.show_player_stats then
...
u.uiplayerstats = task.spawn(function()
    while true do
        task.wait(.5)
...
Z.AgeBreakMachine = {
    ...
    StatusText = "",
    remotes = {},
    STATES = { EMPTY = "EMPTY", SUBMITTED = "SUBMITTED", RUNNING = "RUNNING", READY_TO_CLAIM = "READY_TO_CLAIM", UNKNOWN = "UNKNOWN" },
    SetTextAgebreak = function(
        G)
        Z.TEXT_AGEBREAK = G
        if Z.agebreak_lbl then Z.agebreak_lbl:SetText(G) end
    end,
    LoadRequiredDataAgeBreak = function()
    ...
    BuildAgeBreakRejoinText = function()
        local G = Z.AgeBreakMachine
        G.AgebreakDataLive()
        local V = G.Data
        ...
        if not V or not V.SubmittedPet then
            return
            "<b><font color=\'#FFD166\'>[AgeBreak Rejoin]</font></b>\n<font color=\'#FF6B6B\'>No pet inside the machine.</font>"
        end
    ...
    GetPetInfo = function()
        ...
        return {
            UUID = y.UUID,
            PetType = y.PetType or "?",
            Name = j.Name or "?",
            Level = tonumber(j.Level) or 0,
            BaseWeight =
                tonumber(j.BaseWeight) or 0,
            HatchedFrom = j.HatchedFrom or "",
            MutationType = j.MutationType or "",
            IsFavorite = j
                .IsFavorite == true
        }
    end,
    ...
    AutoRejoinLoop = function()
        ...
        task.wait(2)
        if V.TradeWorldController and V.TradeWorldController.TeleportToMainWorld then
...
J.SendLiveWebhook = function(G, y)
    ...
    local Z, j = pcall(function()
        ...
        if j then
            j({ Url = y, Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = Z })
        else
            V
                .HttpService:PostAsync(y, Z)
        end
    end)
...
q.finder = {
    UpdateStatusPetFound = function(G)
    ...
    GetPlayerNameUsingUserid = function(G)
        if G then
    ...
    GetBoothListings = function()
        ...
        if j then
            for V, y in pairs(i.Booths) do
                ...
                if E then
                    if E.Listings then
                        for V, y in pairs(E.Listings) do
                            local Z = y.Type
                            local j = y.Price
                            local i = y.ItemId
                            local J = E.Items[i]
    ...
    FindSellerUsingPetName = function(G)
...
u.ListSystem = {
    ...
    GetAllFruitsForListing = function()
        local G = {}
    ...
    CreateListingPet = function(G, y) V.GameEvents.TradeEvents.Booths.CreateListing:InvokeServer("Pet", y, G) end,
    CreateListingHoldable = function(
        G, y)
        V.GameEvents.TradeEvents.Booths.CreateListing:InvokeServer("Holdable", y, G)
    end,
    DeleteListingPet = function(
        G)
        local y = G:gsub("[{}]", "")
...
local jJ = y:CreateWindow({
    Title = Z.GetTextUserHubPower(),
    Footer = J.GetFooterInfo(),
...
Z.AgeBreakUi = function()
    local G = jJ:AddTab({
        Name = "<font color=\'#FFAA00\'>Age Break</font>",
        Description = "Age Break Machine Status",
        Icon =
        "timer"
    })
    local V = G:AddLeftGroupbox("Age Break Machine", "timer", false)
    if V then
        Z.agebreak_lbl = V:AddLabel({
            Text =
            "<b><font color=\'#7CFF8A\'>\226\143\179 Age Break</font></b>\n<font color=\'#D7E1FF\'>Loading machine status...</font>",
            DoesWrap = true
        })
        V:AddDivider()
    end
        ...
        j.dd_list_pets = gpSystem:AddDropdown("dd_list_pets",
            {
                ...
                Callback = function(G)
                    ...
                    local c = #V
                    if c > i then
                        j.dd_list_pets:SetValue(GJ(Z.pet_select_for_list))
                    else
        ...
        gpSystem:AddToggle("pausetoggleallsystems",
            {
                ...
                Default = false,
                Tooltip =
                "Pause every system except hatching and mutation.",
                Callback = function(G) end
            })
    end
    local T = i:AddButton({
...
Z.TeleportUi = function()
    local G = o()
    local V = jJ:AddTab({
        Name = "<font color=\'#FFAA00\'>Teleport</font>",
    ...
    if y then
        ...
        y:AddButton({
            Text = "\240\159\154\128 <font color=\'#7CFF8A\'><b>Teleport</b></font>",
            Func = function()
                if V == "" then
        ...
        y:AddDivider()
        y:AddButton({
            Text = "\240\159\147\140 Copy Current Job ID",
            Func = function()
...
Z.BuyListUi = function()
    ...
    if V then
        ...
        g = V:AddInput("input_distancetMinBaseWeight",
            {
                ...
                Placeholder =
                "e.g 1",
                Tooltip = "Filters to show minimum base weight",
                Callback = function(G)
                    local V = f(G)
        ...
        local function E()
            for G, V in ipairs(Z.DynamicScanButtons) do
        ...
        V:AddToggle("manualscanshowbaseweight",
            {
                Text = "\240\159\143\139\239\184\143 Show BaseWeight",
                Default = r.manual_showbaseweight,
                Tooltip =
                "Shows base weight or visual weight",
                Callback = function(G)
        ...
        V:AddButton({
            Text = "\240\159\154\128 Find Server",
            Func = function()
...
Z.BoothListUi = function()
        ...
        G = i:AddDropdown("dd_fruitlistxshowcase",
            {
                ...
                Default = {},
                Multi = true,
                Text = "\240\159\141\137 Showcase Fruit",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        G = j:AddDropdown("dd_displaypets",
            {
                ...
                Default = {},
                Multi = true,
                Text = "\240\159\166\150 ShowCase Pets",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
    ...
    if V then
        local G = V:AddDropdown("dd_skin_list",
            {
                ...
                Default = {},
                Multi = true,
                Text = "\240\159\167\176 Booth Skins",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        V:AddToggle("toggleAutoClaimEquip",
            {
                Text = "\226\154\148\239\184\143Claim Booth",
                Default = r.auto_claim_booth,
                Tooltip = "Auto claim booth",
                Callback = function(
                    G)
        ...
        V:AddToggle("toggleAutoClaimEquipPet",
            {
                Text = "\240\159\166\150Equip Pet/Fruit",
                Default = r.auto_equip_big_pet,
                Tooltip = "Equips a pet or fruit",
                Callback = function(
                    G)
        ...
        V:AddToggle("toggleAutoClaimEquipBooth",
            {
                Text = "\240\159\147\161Teleport To Booth",
                Default = r.teleport_to_booth,
                Tooltip =
                "Teleports behind the booth",
                Callback = function(G)
        ...
        V:AddToggle("togglepromotion",
            {
                Text = "\240\159\151\168\239\184\143<font color=\'#00FF9B\'>Promote Listings</font>",
                Default = r
                    .auto_promote_listing,
                Tooltip = "Sends promotion messages randomly.",
                Callback = function(G)
        ...
        V:AddToggle("toglleJoinnewserver",
            {
                Text = "\240\159\147\161Join New Server",
                Default = r.joinnewserver,
                Tooltip =
                "Joins new server every X min",
                Callback = function(G)
                    r.joinnewserver = G
                    W()
                end
...
Z.PetListUI = function()
    ...
    if T then
        ...
        T:AddLabel({
            Text =
            "\226\154\160\239\184\143 Fixes easter bug. Equips fruits to correct the KG and unequips quickly.",
            DoesWrap = true
        })
        G = T:AddToggle("fixbugeastertoggle",
            {
                Text = "\226\154\170<font color=\'#00FF9B\'>Equip Pets Auto</font>",
                Default = r.fixkgbug_easter,
                Tooltip =
                "Equips and unequip pets to fix easter kg bug",
    ...
    if J then
        ...
        G = J:AddButton({
            Text = "\240\159\159\162 Start Removing",
            DisabledTooltip = "Premium Feature",
            Func = function()
                u.StartRemoveListingFrutis()
            end
        })
    ...
    if i then
        ...
        local j = i:AddDropdown("_shovelwhitelistfruits",
            {
                ...
                Default = {},
                Multi = true,
                Text = "\240\159\140\136 Mutations",
                Tooltip =
                "Fruits matching will be listed.",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(G)
                    if G == nil then return end
        ...
        T = i:AddInput("input_min_weightfruitx",
            {
                Text = c(),
                Default = r.sellfruit.fruit_min_weight,
                Numeric = true,
                AllowEmpty = true,
                Finished = true,
                ClearTextOnFocus = false,
        ...
        d = i:AddInput("input_max_weightfruit",
            {
                Text = J(),
                Default = r.sellfruit.fruit_max_weight,
                Numeric = true,
                AllowEmpty = true,
                Finished = true,
                ClearTextOnFocus = false,
        ...
        q = i:AddInput("input_tokenpricefruit",
            {
                Text = u(),
                Default = r.sellfruit.fruit_price,
                Numeric = true,
                AllowEmpty = true,
                Finished = true,
                ClearTextOnFocus = false,
        ...
        local g = i:AddToggle("toggleAllowFavlistfruit",
            {
                Text = "\226\157\164\239\184\143 Auto Unfav",
                Default = r.sellfruit.fruit_auto_fav,
                Tooltip =
                "Auto Unfav and list",
                Callback = function(G)
        ...
        E = i:AddToggle("toggleEnableGifttingfruit",
            {
                Text = "\226\154\161Enable Fruit-List",
                Default = r.sellfruit.is_fruit_enabled,
                Tooltip =
                "When enabled it lists items based on filters",
                DisabledTooltip = "Premium Feature",
                Callback = function(G)
                    if r.auto_list_enabled and G == true then
    ...
    if V then
        ...
        N = V:AddToggle("toggleEnableGiftting",
            {
                Text = "\226\154\161Enable AutoList",
                Default = r.auto_list_enabled,
                Tooltip =
                "When enabled it lists items based on filters",
                DisabledTooltip = "Premium Feature",
                Callback = function(G)
                    if r.sellfruit.is_fruit_enabled and G == true then
...
Z.FindUiTab = function()
    ...
    if V then
        ...
        local g
        local function E(G)
            if not r.find_settings[G] then
                r.find_settings[G] = { enabled = false, weight_min = .86, weight_max = 12.86, min_level = 1, max_level = 125, price = 3, max_keep = 300, is_weightagebased = false, big_weight_max = 112 }
            else
        ...
        j = V:AddToggle("Toggle_Alt_EnablePet",
            {
                Text = "\226\154\161 Enable Pet",
                Default = false,
                Tooltip = "Toggle finding for the pet selected above",
                Callback = function(
                    V)
                    if not G then return end
                    E(G)
        ...
        i = V:AddInput("Input_Alt_MinWeight",
            {
                Text = "\240\159\159\162 Min BaseWeight",
                Default = 0,
                Numeric = true,
                AllowEmpty = true,
                Finished = true,
        ...
        c = V:AddInput("Input_Alt_MaxWeight",
            {
                Text = "\240\159\148\180 Max BaseWeight",
                Default = 0,
                Numeric = true,
                AllowEmpty = true,
                Finished = true,
        ...
        u = V:AddInput("input_maxkeeppetlimit",
            {
                Text = "\226\154\160\239\184\143Pet Count Limit",
                Default = 0,
                Numeric = true,
                AllowEmpty = true,
                Finished = true,
                ClearTextOnFocus = false,
                Tooltip =
                "Will not buy anymore if your backpack has this amount.",
                Placeholder = "e.g 100",
                Callback = function(V)
                    if not G then return end
                    local Z = z(V)
        ...
        V:AddLabel({
            Text =
            "\226\132\185\239\184\143 When this is enabled BaseWeight and Levels are ignored. Instead it will snipe based on the visual weight you see! example 63.45KG",
            DoesWrap = true
        })
        V:AddDivider()
        ...
        q = V:AddToggle("toggleBigWeight_enabled",
            {
                Text = "\240\159\159\162Enable Visual Weight",
                Default = false,
                Tooltip =
                "When this is enabled. system will not use base weight or levels. Instead will use the weight based on age or what you can see on the pet.",
                Callback = function(
                    V)
                    if not G then return end
                    E(G)
    ...
    if j then
        ...
        G = j:AddDropdown("dd_find_petlist",
            {
                ...
                Default = {},
                Multi = true,
                Text = "\240\159\166\150 Find Pets",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        y = j:AddToggle("toggleEnableFinder",
            {
                Text = "\226\154\161Enable Finder",
                Default = r.finder.find_enabled,
                DisabledTooltip = "Premium Only",
                Tooltip =
                "When enabled it finds targets",
                Callback = function(G)
        ...
        i = j:AddToggle("toggletsameserver",
            {
                Text = "\240\159\147\161Current Server Buying Mode",
                Default = r.sameserver_buyonly,
                DisabledTooltip =
                "Premium Only",
                Tooltip =
                "When enabled it will only scan and buy on current server and will never teleport. [Finder must be enable for this to work]",
                Callback = function(
                    G)
        ...
        for Z, j in ipairs(J.EggDataSet) do
            ...
            for G, V in ipairs(J) do
                ...
                local j = q:AddToggle("Toggle_" .. (i .. ("_" .. Z)),
                    {
                        Text = Z,
                        Default = r.find_settings[Z].enabled,
                        Tooltip = "Filter: " .. Z,
                        Callback = function(G)
                ...
                T = c:AddInput("Min_" .. Z,
                    {
                        Text = J(),
                        Default = r.find_settings[Z].weight_min or 0,
                        Numeric = true,
                        AllowEmpty = true,
                        Finished = true,
                        ClearTextOnFocus = false,
                ...
                u = c:AddInput("Max_" .. Z,
                    {
                        Text = d(),
                        Default = r.find_settings[Z].weight_max or 0,
                        Numeric = true,
                        AllowEmpty = true,
                        Finished = true,
                        ClearTextOnFocus = false,
                ...
                E = c:AddInput("Price_" .. Z,
                    {
                        Text = g(),
                        Default = r.find_settings[Z].price or 0,
                        Numeric = true,
                        AllowEmpty = true,
                        Finished = true,
                        ClearTextOnFocus = false,
...
Z.SettingsUi = function()
    ...
    if i then
        i:AddToggle("toggleacuirejoin",
            {
                Text = "Auto Reconnect",
                Default = r.enable_auto_reconnect,
                Tooltip = "Reconnect if dc",
                Callback = function(
                    G)
                    r.enable_auto_reconnect = G
                end
            })
    end
end
...
J.EquipFruitOrPet = function()
    if not i.IsToolHeldAny() then
...
u.taskboothclaimer = task.spawn(function()
    while true do
        task.wait(4)
...
u.task_sale_check_restart = task.spawn(function()
    while true do
        task.wait(20)
        ...
        if type(j) == "table" then
            ...
            if V and (i and c) then
                local G = {
                    uuid = tostring(V),
                    mut = j.mut,
                    age = tonumber(j.level) or 0,
                    bw = tonumber(j.weight) or 0,
                    nickn =
                        tostring(j.petnickname or ""),
                    ptype = tostring(i),
                    pr = tostring(c),
                    i = tostring(d),
                    fav = T
...
u.ScanForSpecificPet = function()
    ...
    for G, i in pairs(G) do
        if y or V[i.petname] then
            local V = i.weight
...
u.finder_loop = task.spawn(function()
    while true do
        task.wait(5)
        if not Z.GetCheckIfPro() then break end
        ...
        task.wait(4)
        if #G == 0 then
            if not r.sameserver_buyonly then
...
u.task_listing_lister = task.spawn(function()
    while true do
        task.wait(1)
        if not Z.GetCheckIfPro() then break end
...
u.task_listing_fruits = task.spawn(function()
    while true do
        task.wait(1)
        if not Z.GetCheckIfPro() then break end
        local G = u.ListSystem.UpdateStatusUI
...
function OnDcMd()
    (game:GetService("GuiService")).ErrorMessageChanged:Connect(function()
        ...
        if G and G ~= "" then
            ...
            if not Z.prevent_rc then
                if r.enable_auto_reconnect then
                    pcall(function()
